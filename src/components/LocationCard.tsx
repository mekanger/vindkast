...
const DISPLAY_HOURS = ["10:00", "12:00", "14:00", "16:00"];

export const LocationCard = ({ data, onRemove }: LocationCardProps) => {
            ...
            <tbody>
              {forecasts.map((day) => (
                <tr key={day.date} className="border-b border-border/50 last:border-0">
                  ...
                  {DISPLAY_HOURS.map((hour) => {
                    const forecast = day.hours.find(h => h.time === hour);
                    return (
                      <td key={hour} className="py-3 px-2 text-center">
                        {forecast ? (
                          <div className="flex flex-col items-center gap-1">
                            <WindSpeedBadge speed={forecast.speed} />
